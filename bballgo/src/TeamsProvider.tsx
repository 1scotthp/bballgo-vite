      ...
      if (team.teamAbbreviation === team1Abbr) {
        ...
        return {
          ...
          roster: team.roster
            ...
            .concat(
              teams
                .find((t) => t.teamAbbreviation === team2Abbr)
                ?.roster.filter((player) => team2Players.includes(player.name))
                .map((player) => ({ ...player, teamAbbr: team1Abbr })) || []
            ),
        };
        ...
        return {
          ...
          roster: team.roster
            ...
            .concat(
              teams
                .find((t) => t.teamAbbreviation === team1Abbr)
                ?.roster.filter((player) => team1Players.includes(player.name))
                .map((player) => ({ ...player, teamAbbr: team2Abbr })) || []
            ),
        };
      }
      return team;
...
export function calculateGameResult(scoreBoard: ScoreBoard): {
  winner: string;
  loser: string;
  scores: { [teamAbbr: string]: number };
  poss: number;
} {
...
function calculatePlayerStats(boxScores: ScoreBoard[]): {
  [playerName: string]: PlayerBoxScore;
} {
  ...
  boxScores.forEach((game) => {
    Object.values(game.boxScore).forEach((entry) => {
      ...
      playerStats[entry.name].threePointShotsTaken +=
        entry.threePointShotsTaken;
...
function calculateTeamStats(boxScores: ScoreBoard[]): {
  [teamAbbr: string]: TeamsStandingStats;
} {
