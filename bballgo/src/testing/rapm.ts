...
// //     }));
// // }
// function combineAndTransformData(rawData: number[][], numPlayers: number, playerIDMap: Record<number, number>): { x: number[], y: number, weight: number }[] {
//     const combinedData = new Map<string, { x: number[], totalY: number, count: number }>();

//     rawData.forEach(row => {
//         const remappedRow = row.map((id, index) => index === 0 ? id : (playerIDMap[id] || 0));
//         const key = remappedRow.slice(1, 11).sort().join(',');
...
//         const data = combinedData.get(key);
//         if (data) {
//             data.totalY += remappedRow[0]; // Sum outcomes
//             data.count += 1; // Count possessions

//             for (let i = 1; i <= 5; i++) {
//                 const remappedPlayerID = remappedRow[i];
//                 if (remappedPlayerID !== 0) {
//                     data.x[remappedPlayerID] = 1;
//                 }
//             }
//             for (let i = 6; i <= 10; i++) {
//                 const remappedPlayerID = remappedRow[i];
...
//         [row[0]].concat(row.slice(1, 11).map((playerID, index) => {
...
//             }
//             return 100; // Map players with less than minOccurrences (or ID = 0) to 0
//         }))
//     );
...
//         }
//     });
//     // rawData is original possessions 2D array

//     console.log(maxPlayerID);
//     const MIN_OCCURRENCES = 200;

//     // player counts is map from player ID to occurences
...
//     console.log(filteredData.slice(0, 20), playerIDMap);
...
//     // console.log("HERE", filteredData, rawData);

//     const transformedData = combineAndTransformData(rawData, maxPlayerID + 1, playerIDMap);

//     console.log("len", rawData.length, maxPlayerID, transformedData);
//     // console.log("transformed", transformedData.length)
//     const lambda = 10; // Regularization parameter, adjust as needed
...
//     console.log("MAPS", playerIDMap, reverseMap);
...
//     for (let i = 0; i < coefficients.length; i++) { // Start from 1 since 0 is mapped to players with few occurrences
//         const originalID = reverseMap[i];
//         console.log("origID", originalID, "i", i, coefficients[i]);
//         if (originalID !== undefined) {
//             retCoefficients[originalID] = coefficients[i];
//         }
//     }
    
//     return retCoefficients;
// }
